...
`async` functions implicitly return a Promise object themselves, and whatever you `return` from the `async` function will be the promise's resolution. If instead you `throw` from the body of an `async` function, that will be how your async function rejects.
